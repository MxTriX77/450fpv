...
- The reference set started as 8 clips (1920×1080, about 30 fps, 4–25 s each, about 100 s in total) plus 3 stills. The pilot later added a night clip (L), 3 destroyed-vehicle photos (M–O, AVIF) and a severe-wind clip (P, analysed in `study-wind-reference`). The clips look like messenger re-encodes, most likely recordings of goggles or a DVR.
...
- **Letters are stable.** The tool reads the existing `index.md` and keeps every letter; new files get the next free letters. Photos (first I–K, later M–O) are indexed so the notes can cite them. Formats Blender shows directly aren't re-encoded; AVIF gets a lossless `photo.png` copy inside `reference/_frames/<letter>/`.
