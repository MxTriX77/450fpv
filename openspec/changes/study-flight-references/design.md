...
- **Scan every frame.** About 2,900 frames in total. Metrics are computed on a downscaled copy (about 480×270) to keep runtime down: high-pass energy for noise, variance of row means for stripes, mean absolute difference for change. Outliers are values above the clip's median plus k × MAD, with k configurable. Sampling was rejected because single-frame flashes fall between the samples.
