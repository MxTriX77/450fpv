...
| Q4 | Mass and props | A **10-inch heavy fiber quad of the reference class** (see `docs/reference-notes/airframes.md`): fiber coil, cargo and battery in the class ranges. Sortie specifics are deliberately not recorded (OPSEC, pilot's decision 2026-09-24). | Use the class mass range from `airframes.md`, and propagate it to the thrust margin and inertia. |
| Q5 | Height and speed | **Varied height** | Keep the height-dependent turbulence (surface layer, tree-belt shear) as a range. |
| Q6 | Flight duration | The 47 s clip was **the toughest part** of a longer flight. | The §6 targets describe the worst stretch of a severe-wind flight, not a whole flight. Say so, and consider a milder "typical windy" band. |
...
| Q8 | Cause of the picture losses | Unknown; the pilot was far from the drone. **Hypothesis: wind shakes the optic fiber, making it noisier and flashier, and trees plus wobbling stretch it.** | Feed dropouts (N12) and extra noise get a **fiber tension and vibration** driver, strongest in wind and near trees. This links the tether model to the video feed. |
