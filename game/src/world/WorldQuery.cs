...
public sealed partial class WorldQuery
{
    ...
    void Sample(double x, double z, out GroundSample g, bool withMat)
    {
        ...
        // Terrain: HeightmapTerrain splits each cell (a, b / c, d) into triangles a-b-c (u + v ≤ 1) and b-d-c.
        ...
        double terrain, gx, gz;
        if (u + v <= 1.0)
        {
            terrain = h00 + u * (h10 - h00) + v * (h01 - h00);
            gx = (h10 - h00) * _perHeightStep;
            gz = (h01 - h00) * _perHeightStep;
        }
        else
        {
            terrain = h11 + (1.0 - u) * (h01 - h11) + (1.0 - v) * (h10 - h11);
            gx = (h11 - h01) * _perHeightStep;
            gz = (h11 - h10) * _perHeightStep;
        }
        if (tx != x)
            gx = 0; // outside the map the edge height continues flat
        if (tz != z)
            gz = 0;
        ...
        if (b.Uniform)
            relief = Relief(b.S0, x, z, out rdx, out rdz); // the weights sum to 1 and their slopes cancel
        else
            relief = BlendedRelief(in b, x, z, out rdx, out rdz);
        gx += rdx;
        gz += rdz;
        ...
        double m0 = Mat(in b, 0, x, z), m1 = Mat(in b, 1, x, z), m2 = Mat(in b, 2, x, z), m3 = Mat(in b, 3, x, z);
    ...
    double BlendedRelief(in Blend b, double x, double z, out double dx, out double dz)
    {
        double r0 = Relief(b.S0, x, z, out double x0, out double z0);
        double r1, x1, z1, r2, x2, z2, r3, x3, z3;
        if (b.S1 == b.S0) (r1, x1, z1) = (r0, x0, z0);
        else r1 = Relief(b.S1, x, z, out x1, out z1);
        if (b.S2 == b.S0) (r2, x2, z2) = (r0, x0, z0);
        else if (b.S2 == b.S1) (r2, x2, z2) = (r1, x1, z1);
        else r2 = Relief(b.S2, x, z, out x2, out z2);
        if (b.S3 == b.S0) (r3, x3, z3) = (r0, x0, z0);
        else if (b.S3 == b.S1) (r3, x3, z3) = (r1, x1, z1);
        else if (b.S3 == b.S2) (r3, x3, z3) = (r2, x2, z2);
        else r3 = Relief(b.S3, x, z, out x3, out z3);
        double inv = _perCell;
    ...
    /// surface. The noise is evaluated once per distinct surface.
    double Mat(in Blend b, int kind, double x, double z)
    {
        double n0 = MatNoise(b.S0, kind, x, z);
        double n1 = b.S1 == b.S0 ? n0 : MatNoise(b.S1, kind, x, z);
        double n2 = b.S2 == b.S0 ? n0 : b.S2 == b.S1 ? n1 : MatNoise(b.S2, kind, x, z);
        double n3 = b.S3 == b.S0 ? n0 : b.S3 == b.S1 ? n1 : b.S3 == b.S2 ? n2 : MatNoise(b.S3, kind, x, z);
    ...
    double MatNoise(byte s, int kind, double x, double z)
    {
        CoverParams c = _byIndex[s].Cover[kind];
        if (c == null || !c.HasMat)
            return 0;
        double noise = ValueNoise(_matKey[s * 4 + kind], x, z, _nodeScale[s], out _, out _);
        return c.MatDepthMin + (c.MatDepthMax - c.MatDepthMin) * (0.5 + 0.5 * noise);
    }

    /// Micro-relief plus ridges of one surface at (x, z), with its gradient.
    double Relief(byte index, double x, double z, out double dx, out double dz)
    {
        double scale = _reliefScale[index];
        double h = scale * ValueNoise(_reliefKey[index], x, z, _nodeScale[index], out dx, out dz);
        dx *= scale;
        dz *= scale;
        SurfaceParams s = _byIndex[index];
        if (s.RidgeAmplitude > 0)
        {
            // A C² periodic profile with crests at whole phases: 1 − 2·smoothstep(2q), q = distance to the crest.
            double nx = _ridgeNx[index], nz = _ridgeNz[index];
            double phase = (x * nx + z * nz) / s.RidgeSpacing + _ridgePhase[index];
            double f = phase - Math.Floor(phase);
            double q = f <= 0.5 ? f : 1.0 - f;
            h += s.RidgeAmplitude * (1.0 - 2.0 * DetMath.Smooth3(2.0 * q));
            double slope = s.RidgeAmplitude * -4.0 * DetMath.Smooth3Slope(2.0 * q) * (f <= 0.5 ? 1.0 : -1.0) / s.RidgeSpacing;
            dx += slope * nx;
            dz += slope * nz;
        }
        return h;
    ...
    /// Value noise in [−1, 1] with nodes 1/`scale` apart on the world grid, quintic fades, and its gradient.
    static double ValueNoise(uint key, double x, double z, double scale, out double dx, out double dz)
    {
        double px = x * scale, pz = z * scale;
        double fx = Math.Floor(px), fz = Math.Floor(pz);
        int ix = (int)fx, iz = (int)fz;
        double tx = px - fx, tz = pz - fz;
        double v00 = Node(ix, iz, key), v10 = Node(ix + 1, iz, key), v01 = Node(ix, iz + 1, key), v11 = Node(ix + 1, iz + 1, key);
        double a = DetMath.Fade5(tx), b = DetMath.Fade5(tz);
        double cross = v00 - v10 - v01 + v11;
        dx = DetMath.Fade5Slope(tx) * (v10 - v00 + b * cross) * scale;
        dz = DetMath.Fade5Slope(tz) * (v01 - v00 + a * cross) * scale;
        return v00 + a * (v10 - v00) + b * (v01 - v00) + a * b * cross;
    }

    static double Node(int x, int z, uint key) => DetMath.Unit(DetMath.Hash(x, z, key)) * 2.0 - 1.0;

    /// The deepest pitfall at (x, z): depth ≥ 0, its gradient and id (0 = none). Each 1 m cell holds at most one
    /// candidate, which exists with the probability of the density of the surface under its centre. Its floor is flat
    /// and its wall is a smoothstep over the outer 25 % of the radius.
    double Pitfall(double x, double z, out double dx, out double dz, out uint id)
    {
        ...
        for (int cz = z0; cz <= z1; cz++)
        {
            ...
            {
                uint h = DetMath.Hash(cx, cz, _pitKey);
                double chance = DetMath.Unit(h);
                if (chance >= _pitMaxDensity)
                    continue;
                double px = (cx + DetMath.Unit(DetMath.Draw(h, 1))) * PitCell, pz = (cz + DetMath.Unit(DetMath.Draw(h, 2))) * PitCell;
                SurfaceParams s = _byIndex[_surface[OwnCell(px, pz)]];
                if (chance >= s.PitDensity)
                    continue;
                double radius = s.PitRadiusMin + (s.PitRadiusMax - s.PitRadiusMin) * DetMath.Unit(DetMath.Draw(h, 3));
